...
class AuthService {
  ...
  try {
    final response = await _dio.post(
      ...
      options: Options(
        validateStatus: (status) {
          print('🚦 [AuthService] Login Server responded with HTTP Status Code: $status');
    ...
    print('💥 [AuthService] Unexpected code runtime error inside login call: $e');
    return null;
  }
}
 ...
 Future<Response?> register(String name, String email, String password) async {
  ...
  try {
    final response = await _dio.post(
      ...
      data: {
        'email': email, // 🟢 Sends the email text under the exact key your backend expects
        'otpCode': otpCode,
      },
      options: Options(
        validateStatus: (status) {
          print('🚦 [AuthService] OTP Server responded with HTTP Status Code: $status');
          // Accept 200, 201, 400, and 404 without crashing Dio!
          return status == 200 || status == 201 || status == 400 || status == 404 || status == 500;
        },
      ),
    );
    ...
    print('❌ [AuthService] DioException during OTP verification: ${ex.message}');
    return ex.response;
  } catch (e) {
